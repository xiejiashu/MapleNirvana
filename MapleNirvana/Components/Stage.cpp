...
namespace ms
{
	...
	void Stage::load_map(int32_t mapid)
	{
		...
		{
			int prefix_val = mapid / 100000000;
			src = nl::nx::Map["Map"]["Map" + prefix][strid + ".img"];

			// if (prefix_val < 2)
			// 	src = nl::nx::Map001["Map"]["Map" + prefix][strid + ".img"];
			// else if (prefix_val == 2)
			// 	src = nl::nx::Map002["Map"]["Map" + prefix][strid + ".img"];
			// else
			// 	src = nl::nx::Map2["Map"]["Map" + prefix][strid + ".img"];
		}

		if (!src)
		{
			LOG(LOG_ERROR, "Failed to load map source for ID: " << mapid);
		}
